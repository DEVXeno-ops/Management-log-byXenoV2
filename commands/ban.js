  ...
  async execute(interaction) {
    ...
    try {
      ...
      return interaction.reply({
        embeds: [new EmbedBuilder().setColor('Red').setTitle('❌ ไม่พบผู้ใช้').setDescription('ไม่สามารถดึงข้อมูลผู้ใช้จากเซิร์ฟเวอร์ได้!')],
        ephemeral: true
      });
    }
    ...
    if (!isOwner && !isAdmin) {
      return interaction.reply({
        embeds: [new EmbedBuilder().setColor('Red').setTitle('🚫 คุณไม่มีสิทธิ์ใช้คำสั่งนี้').setDescription('เฉพาะเจ้าของเซิร์ฟเวอร์หรือแอดมินเท่านั้นที่สามารถใช้คำสั่งนี้')],
        ephemeral: true
      });
    }

    // ตรวจสอบว่าผู้ใช้ไม่ใช่เจ้าของเซิร์ฟเวอร์, แอดมิน, หรือผู้ใช้ตัวเอง
    if ([interaction.guild.ownerId, ...adminRoleIds].includes(member.id) || member.id === executor.id) {
      return interaction.reply({
        embeds: [new EmbedBuilder().setColor('Orange').setTitle('⚠️ ไม่สามารถแบนผู้ใช้คนนี้ได้').setDescription('ไม่สามารถแบนเจ้าของเซิร์ฟเวอร์, แอดมิน หรือผู้ใช้ตัวเอง')],
        ephemeral: true
      });
    }

    // ตรวจสอบบอทมีสิทธิ์แบน
    if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.BanMembers)) {
      return interaction.reply({
        embeds: [new EmbedBuilder().setColor('Red').setTitle('⚠️ บอทไม่มีสิทธิ์แบน').setDescription('โปรดให้สิทธิ์ **Ban Members** แก่บอท')],
        ephemeral: true
      });
    }

    // ตรวจสอบสิทธิ์ของบอทในเทียบกับผู้ใช้
    if (executor.roles.highest.position <= member.roles.highest.position && interaction.guild.members.me.roles.highest.position <= member.roles.highest.position) {
      return interaction.reply({
        embeds: [new EmbedBuilder().setColor('Red').setTitle('⛔ คุณไม่สามารถแบนผู้ใช้ที่มีสถานะสูงกว่าคุณได้!')],
        ephemeral: true
      });
    }
    ...
    collector.on('collect', async i => {
      if (i.customId === 'confirm_ban') {
          ...
          await user.send({
            embeds: [
              new EmbedBuilder()
                ...
                .setFooter({ text: 'Ban System • โปรดระวังการกระทำในเซิร์ฟเวอร์', iconURL: interaction.client.user.displayAvatarURL() })
                .setTimestamp()
                .setImage('https://your-image-link-here.com') // Add background image or link here
            ]
        ...
        await i.update({
          embeds: [new EmbedBuilder().setColor('Green').setTitle('✅ ยกเลิกการแบน').setDescription(`คำสั่งแบนของ **${user.tag}** ถูกยกเลิก`)],
          components: []
        });
      }
    });
